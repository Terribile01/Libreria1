...
export default function ListenPage({ tracks, activeTrackId, setActiveTrackId }: ListenPageProps) {
  ...
  const activeReading = useMemo(() => {
    return readings.find(r => r.book_id === activeTrackId || r.id === activeTrackId);
  }, [activeTrackId, readings]);
  ...
  const activeTrack = useMemo(() => {
    ...
    if (activeReading && activeReading.book) {
      return {
    ...
    // 3. Fallback
    return tracks[0];
  }, [activeTrackId, tracks, activeReading, extractedText]);

  // Logic to process PDF if active reading has one
  useEffect(() => {
    const processPdf = async () => {
      if (activeReading?.source_type === 'internal' && activeReading.file_path) {
        setIsProcessingPdf(true);
        setExtractedText(null);
        try {
          // 1. Get Signed URL from Supabase
          const url = await BookService.getFileUrl(activeReading.file_path);
          // 2. Extract Raw Text
                ...
                <div className="text-center sm:text-left space-y-2 flex-1">
                  <div className="space-y-1">
                    <span className="px-2 py-0.5 bg-primary/10 text-primary font-sans font-bold text-[9px] tracking-widest uppercase rounded">
                      {activeReading?.source_type === 'internal' ? 'Lettura Documento' : 'Sintonizzazione Attiva'}
                    </span>
